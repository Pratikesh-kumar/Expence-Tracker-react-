...
export const ExpenseContextProvider = (props) => {
  ...
  //         "https://e-commerce-9cfa8-default-rtdb.firebaseio.com/expenses.json",
  //         {
  //           method: "POST",
  //           body: JSON.stringify({
  ...
  const deleteExpenseHandler = (id) => {
    const deleteExpenseItem = async (id) => {
      try {
        const response = await fetch(
          `https://e-commerce-9cfa8-default-rtdb.firebaseio.com/expenses/${id}.json`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
  ...
  const editExpenseHandler = (expenseItem) => {
    const editExpenseItem = async (expenseItem) => {
      try {
        const response = await fetch(
          `https://e-commerce-9cfa8-default-rtdb.firebaseio.com/expenses/${expenseItem.id}.json`,
          {
            method: "PUT",
            body: JSON.stringify({
